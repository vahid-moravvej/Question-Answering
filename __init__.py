from qasystem.qa_model.model import SiameseBiLSTM
from qasystem.qa_model.inputHandler import word_embed_meta_data, create_test_data
from qasystem.qa_model.config import siamese_config
import pandas as pd
import pickle
import glob
...
subject="Corona"
pre_path_files = 'qasystem/qa_model/'+subject+"/"
stop_words_persian_path = 'qasystem/qa_model/Stop_Words.txt'
###### English Data ######################
def Read_Json(json_file,text_file):
    question=""
    with open(json_file) as f:
        data = json.load(f)
        for i in range(len(data["data"])):
                question=question+data["data"][i]["question"]+" "+data["data"][i]["answers"][0]
                question = question+"\n"
...
def Train(languege='Persian'):
        ...
        #question_pre = question_pre[0:20]
        #answering_pre = answering_pre[0:20]
        #is_similar = is_similar[0:20]
...
def Ask_Question(question,languege='Persian'):
    ...
    if(languege=='Persian'):
        ...
        return result_list
def Accuracy():
    Questions = load_data(pre_path_files + 'Persian/ListQuestion')
    Answering = load_data(pre_path_files + 'Persian/ListAnswering')
    co=0
...
#Create_Dataset(question,answering,'Persian')
#Word_Embedding('Persian')
#Train('Persian')
#Accuracy()
#print(Ask_Question(' امکان اخذ نوبت در سامانه غیرحضوری'))


# import xlrd
# workbook = xlrd.open_workbook("/srv/Projects/WebProject/Summarization/SummarizationBackend/cornaquestion (1).xls")
#
# worksheet = workbook.sheet_by_name(pd.ExcelFile("/srv/Projects/WebProject/Summarization/SummarizationBackend/cornaquestion (1).xls").sheet_names[0])
# Q=[]
# A=[]
# st=""
# for i in range(1,199):
#     if(worksheet.cell(i, 10).value==''):
#         continue
#     c=int(worksheet.cell(i, 10).value)
#     s=1
#     for j in range(c):
#         st=st+worksheet.cell(i, s).value
#         st = st + worksheet.cell(i, 0).value
#         st=st+"\n"
#         A.append(worksheet.cell(i, 0).value)
#         Q.append(worksheet.cell(i, s).value)
#         s=s+1